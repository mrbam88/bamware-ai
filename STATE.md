...
| Date | What |
|---|---|
| 2026-08-19 | **BrewDesk App Store package and native TestFlight rail verified.** App Store Connect record exists for `io.bamware.brewdesk`; evidence-first metadata, review notes, 4.3 preflight, five opaque 1320×2868 screenshots, and deterministic Release screenshot automation are committed. Native fastlane CI at `bamware-brewdesk@cbbc25f` imports the existing distribution certificate, creates/downloads the app-specific profile, assigns the next build number, signs with Xcode 26.5, uploads through `pilot`, and waits for Apple processing. The protected `production` environment is restricted to `main` but has no secrets yet; Baat's credentials live inside EAS and were not copied. Package tests, full Release app/UI tests, iPad Air iPhone-compatibility smoke, development-workspace Release build, identity check, and unsigned device archive pass. Remaining human gates: add the existing ASC key/certificate as environment secrets, dispatch TestFlight, production logging confirmation, physical-device location states, questionnaires, and submission. |
